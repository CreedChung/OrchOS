...
export function AvatarUpload({ agentId, avatarUrl, name, size = "md", onUploaded }: AvatarUploadProps) {
  ...
  return (
    ...
    >
      {avatarUrl ? (
        ...
        <span className={cn("text-xs font-bold text-primary", size === "sm" ? "text-[10px]" : size === "lg" ? "text-lg" : "")}>
      ...
      <div className={cn(
        ...
        {uploading ? (
          <HugeiconsIcon icon={Loading01Icon} className={cn(iconSizeMap[size], "text-background animate-spin")} />
        ) : (
