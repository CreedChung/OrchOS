...
export abstract class SkillService {
  ...
  static async get(db: AppDb, id: string): Promise<SkillProfile | undefined> {
  ...
  static async list(
    ...
    const allRows = (await db.select().from(skills).all()).map(SkillService.mapRow);
  ...
  static async update(
    ...
    const existing = await SkillService.get(db, id);
  ...
  static async delete(db: AppDb, id: string): Promise<boolean> {
    ...
    if (existing.sourceType === "repository" && existing.installPath) {
      await SkillService.removeInstalledSkill(db, existing);
    }
  ...
  static async analyzeRepository(
    ...
    const installTarget = await SkillService.getInstallTarget(db, scope, data.projectId);
    ...
    if (!rootPath) {
      ...
      if (!executor) {
        throw new Error("Executor not available in this environment");
      }
  ...
  static async installFromAnalysis(
    db: AppDb,
    data: {
      analysisId: string;
    ...
    for (const candidate of selected) {
      ...
      const skill = await SkillService.create(db, {
        name: candidate.name,
  ...
  private static async getInstallTarget(
    db: AppDb,
    scope: SkillScope,
    projectId?: string,
  ): Promise<string> {
    if (scope === "project") {
      ...
      if (!ProjectService) {
        throw new Error("ProjectService not available in this environment");
      }
      const project = await ProjectService.get(db, projectId);
      if (!project) {
  ...
  private static async removeInstalledSkill(db: AppDb, skill: SkillProfile) {
    ...
    const ProjectService = await getProjectService();
    const projectRoot =
      skill.projectId && ProjectService
        ? resolve(
            join((await ProjectService.get(db, skill.projectId))?.path || "", ".agents", "skills"),
          )
        : undefined;
