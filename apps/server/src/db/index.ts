...
function migrate(sqlite: Database) {
  sqlite.run("CREATE TABLE IF NOT EXISTS goals (id TEXT PRIMARY KEY, title TEXT NOT NULL, description TEXT, success_criteria TEXT NOT NULL DEFAULT '[]', constraints TEXT NOT NULL DEFAULT '[]', status TEXT NOT NULL DEFAULT 'active', created_at TEXT NOT NULL, updated_at TEXT NOT NULL)")
  ...
  sqlite.run("CREATE TABLE IF NOT EXISTS activities (id TEXT PRIMARY KEY, goal_id TEXT NOT NULL, timestamp TEXT NOT NULL, agent TEXT NOT NULL, action TEXT NOT NULL, detail TEXT, reasoning TEXT)")
  sqlite.run("CREATE TABLE IF NOT EXISTS agents (id TEXT PRIMARY KEY, name TEXT NOT NULL UNIQUE, role TEXT NOT NULL, capabilities TEXT NOT NULL DEFAULT '[]', status TEXT NOT NULL DEFAULT 'idle', model TEXT NOT NULL)")
