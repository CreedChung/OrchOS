...
export const sandboxController = new Elysia({ prefix: "/api/sandbox" })
  ...
  .delete("/vms/:vmId", async ({ params: { vmId } }) => {
  ...
  .post("/vms/:vmId/files/read", async ({ params: { vmId }, body }) => {
  ...
  .post("/vms/:vmId/files/write", async ({ params: { vmId }, body }) => {
  ...
  .post("/vms/:vmId/exec", async ({ params: { vmId }, body }) => {
  ...
  .post("/vms/:vmId/sessions", async ({ params: { vmId }, body }) => {
  ...
  .get("/vms/:vmId/sessions", ({ params: { vmId } }) => {
    const instance = SandboxService.getVM(vmId)
    if (!instance) throw status(404, "VM not found")
  ...
  .post("/sessions/:sessionId/prompt", async ({ params: { sessionId }, body }) => {
  ...
  .post("/sessions/:sessionId/cancel", async ({ params: { sessionId } }) => {
  ...
  .get("/sessions/:sessionId/events", ({ params: { sessionId }, query }) => {
  ...
  .patch("/sessions/:sessionId/permissions", async ({ params: { sessionId }, body }) => {
  ...
  .patch("/sessions/:sessionId/config", async ({ params: { sessionId }, body }) => {
    ...
    if (body.thoughtLevel) await SandboxService.setSessionThoughtLevel(sessionId, body.thoughtLevel)
    return { success: true }
  }, {
    params: t.Object({ sessionId: t.String() }),
