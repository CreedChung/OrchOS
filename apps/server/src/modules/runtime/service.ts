...
export abstract class RuntimeService {
  ...
  static updateEnabled(id: string, enabled: boolean): RuntimeProfile | undefined {
    const result = db.update(runtimes).set({ enabled: String(enabled) }).where(eq(runtimes.id, id)).run()
  ...
  static registerFromDetection(definition: {
    ...
    if (existing) {
      ...
      if (definition.version && existing.version !== definition.version) updates.version = definition.version
  ...
  static async healthCheck(runtimeId: string, options?: { level?: "basic" | "ping" | "full"; prompt?: string }) {
  ...
  static async chat(runtimeId: string, prompt: string): Promise<{
    success: boolean
    output: string
    error?: string
    agentName: string
    ...
    if (!runtime) {
      return { success: false, output: "", error: "Runtime not found", agentName: runtimeId, responseTime: 0 }
    }

    const startTime = Date.now()
    const result = await executor.run(`${runtime.command} -p '${prompt.replace(/'/g, "'\\''")}' 2>&1`, {
      timeout: 120000,
    })
