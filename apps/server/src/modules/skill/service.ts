import { cpSync, existsSync, lstatSync, mkdirSync, mkdtempSync, readFileSync, readdirSync, rmSync } from "fs"
...
const SCRIPT_EXTENSIONS = new Set([".sh", ".bash", ".zsh", ".fish", ".ps1", ".bat", ".cmd", ".command"])
const BINARY_EXTENSIONS = new Set([".exe", ".dll", ".dylib", ".so", ".bin", ".node", ".jar", ".apk"])
const PACKAGE_FILES = new Set(["package.json", "bun.lock", "bun.lockb", "package-lock.json", "pnpm-lock.yaml", "yarn.lock", "requirements.txt"])
...
export abstract class SkillService {
  ...
  static installFromAnalysis(data: {
  ...
  }): { installed: SkillProfile[]; installTarget: string; warnings: string[]; riskLevel: RiskLevel } {
    ...
    const selectedSet = new Set(data.selectedSkills && data.selectedSkills.length > 0
      ? data.selectedSkills
      : cached.analysis.installableSkills.map((candidate) => candidate.relativePath))

    const selected = cached.analysis.installableSkills.filter((candidate) => selectedSet.has(candidate.relativePath))
    ...
    const installed = selected.map((candidate) => {
      const sourceDir = candidate.relativePath === "." ? cached.rootPath : join(cached.rootPath, candidate.relativePath)
  ...
  private static parseSkillCandidate(rootPath: string, dirPath: string, relativePath: string): SkillRepositoryCandidate {
  ...
  private static buildWarnings(stats: RepositoryScanStats, candidates: SkillRepositoryCandidate[]): string[] {
    ...
    if (stats.binaryFiles.length > 0) {
      warnings.push(`Binary files detected: ${stats.binaryFiles.slice(0, 3).join(", ")}${stats.binaryFiles.length > 3 ? "..." : ""}`)
    }
    if (stats.symlinkFiles.length > 0) {
      warnings.push(`Symbolic links detected: ${stats.symlinkFiles.slice(0, 3).join(", ")}${stats.symlinkFiles.length > 3 ? "..." : ""}`)
    }
    if (stats.scriptFiles.length > 0) {
      warnings.push(`Shell or command scripts detected: ${stats.scriptFiles.slice(0, 3).join(", ")}${stats.scriptFiles.length > 3 ? "..." : ""}`)
    }
    if (stats.packageFiles.length > 0) {
      warnings.push(`Package or dependency manifests detected: ${stats.packageFiles.slice(0, 3).join(", ")}${stats.packageFiles.length > 3 ? "..." : ""}`)
    }
    if (stats.workflowFiles.length > 0) {
      warnings.push(`CI workflow files detected: ${stats.workflowFiles.slice(0, 3).join(", ")}${stats.workflowFiles.length > 3 ? "..." : ""}`)
    }
  ...
  private static deriveRiskLevel(stats: RepositoryScanStats, candidates: SkillRepositoryCandidate[]): RiskLevel {
    if (candidates.length === 0 || stats.binaryFiles.length > 0 || stats.symlinkFiles.length > 0) {
      return "high"
    }

    if (stats.scriptFiles.length > 0 || stats.packageFiles.length > 0 || stats.workflowFiles.length > 0 || stats.filesScanned > 500) {
      return "medium"
    }
  ...
  private static buildSummary(candidates: SkillRepositoryCandidate[], stats: RepositoryScanStats, riskLevel: RiskLevel): string {
    const skillPart = candidates.length === 1
