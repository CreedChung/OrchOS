...
function migrate(sqlite: Database) {
  sqlite.run("CREATE TABLE IF NOT EXISTS goals (id TEXT PRIMARY KEY, title TEXT NOT NULL, description TEXT, success_criteria TEXT NOT NULL DEFAULT '[]', constraints TEXT NOT NULL DEFAULT '[]', status TEXT NOT NULL DEFAULT 'active', project_id TEXT, created_at TEXT NOT NULL, updated_at TEXT NOT NULL, FOREIGN KEY (project_id) REFERENCES projects(id))")
