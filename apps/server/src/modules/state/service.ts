...
export abstract class StateService {
  static createState(goalId: string, label: string, status: Status, actions?: string[]): StateEntry {
  ...
  static getStatesByGoal(goalId: string): StateEntry[] {
    return db.select().from(states).where(eq(states.goalId, goalId)).orderBy(asc(states.updatedAt)).all().map(StateService.mapRowToState)
  }
  ...
  static createArtifact(goalId: string, name: string, type: Artifact["type"], status: Status, detail?: string): Artifact {
  ...
  static getArtifactsByGoal(goalId: string): Artifact[] {
    return db.select().from(artifacts).where(eq(artifacts.goalId, goalId)).orderBy(desc(artifacts.updatedAt)).all().map(StateService.mapRowToArtifact)
  }
