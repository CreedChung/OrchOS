...
export abstract class ConversationService {
  static create(data: { title?: string; projectId?: string; agentId?: string; runtimeId?: string }): Conversation {
    ...
    return { id, title: data.title, projectId: data.projectId, agentId: data.agentId, runtimeId: data.runtimeId, createdAt: now, updatedAt: now }
  }
  ...
  static list(): Conversation[] {
    return db.select().from(conversations).orderBy(desc(conversations.updatedAt)).all().map(ConversationService.mapRow)
  }

  static update(id: string, data: { title?: string; projectId?: string; agentId?: string; runtimeId?: string }): Conversation | undefined {
  ...
  static getMessages(conversationId: string): Message[] {
    return db.select().from(messages).where(eq(messages.conversationId, conversationId)).all().map(ConversationService.mapMessageRow)
  }

  static addMessage(conversationId: string, role: "user" | "assistant", content: string, error?: string, responseTime?: number): Message {
  ...
  static async sendAndReply(conversationId: string, userContent: string): Promise<Message> {
    ...
    if (!runtimeId) {
      const errorMsg = "No runtime configured for this conversation. Please select an agent/runtime."
