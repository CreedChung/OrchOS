...
export const executor = {
  ...
  async checkTools(): Promise<Record<string, boolean>> {
    const tools = ["git", "node", "npm", "pnpm", "yarn", "bun", "npm", "pnpm", "yarn"]
  ...
  // Run tests (detect package manager and run appropriate test command)
  async runTests(cwd?: string): Promise<ExecutionResult> {
    // Check for various test commands
    const commands = [
      "bun test",
      "npm test",
      "pnpm test",
    ...
    return {
      success: false,
      output: "",
      error: "No test runner found (tried bun test, npm test, pnpm test, yarn test)",
      exitCode: 127,
    }
  },
